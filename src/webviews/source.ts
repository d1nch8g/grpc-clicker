...
export interface SourceWebViewParameters {
   ...
   * Callback for saving webview to storage and updating trees.
   */
  selectFileCallback: () => Promise<string>;
}

/**
 * Represents wether the current state of connection to show it to user.
 */
export type ConnectionStatus = `NOT_EXECUTED` | `WAITING` | `ERROR` | `SUCCESS`;
...
class SourceWebviewTab {
  ...
  constructor(
    ...
    this.panel.webview.onDidReceiveMessage(async (out) => {
      switch (out.command) {
        case "file":
          this.data.filePath = await this.params.selectFileCallback();
